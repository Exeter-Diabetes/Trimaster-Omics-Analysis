...
data_analysis <- data %>%
  ...
  select(all_of(c("study_id", clinical_features, "resphba1c_SGLT2", "resphba1c_DPP4"))) %>%
...
summary(SGLT2_DPP4_lm)


# Plot results
plot_forest(SGLT2_DPP4_lm, 
            title = "SGLT2 vs DPP4 (Differential Response)",
            save_path = "Plots/SGLT2_v_DPP4_differential_response.png",
            width = 10, height = 5.5)
...
## Regression analysis ----
# (negative is SGLT2, positive is DPP4)
...
# Findings:
## PC8 was significantly associated with differential glycaemic response between SGLT2i and DPP4i
## FABP4 was a top contributor to PC8 and was significantly associated with greater benefit from DPP4i
...
summary_sglt2_dpp4 <- generate_pc_summary_tables(
  output_proteomics = output_proteomics,
  results_drug1 = results_protein_SGLT2,
  results_drug2 = results_protein_DPP4,
  drug1_label = "SGLT2",
  drug2_label = "DPP4"
)
...
# Save summary table
for (pc in names(summary_sglt2_dpp4)) {
  write.csv(summary_sglt2_dpp4[[pc]],
            file = paste0("Output/summary_table_SGLT2_v_DPP4_", pc, ".csv"),
            row.names = FALSE)
}
